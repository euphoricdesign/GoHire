...
export const userSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addMatcher(
        userApi.endpoints.postUser.matchPending,
        (state) => {
          state.loading = true;
          state.error = null;
        }
      ...
      .addMatcher(
        userApi.endpoints.postUser.matchFulfilled,
        (state, action) => {
          state.loading = false;
          state.userDetail = action.payload;
        }
      ...
      .addMatcher(
        userApi.endpoints.postUser.matchRejected,
        (state, action) => {
          state.loading = false;
...
export const { setUserDetail, clearUserDetail, setLoading, setError } = userSlice.actions;
