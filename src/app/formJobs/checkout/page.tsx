...
import { JobsFindData } from "@/types/jobsTypes";
...
const Checkout: React.FC = () => {
  ...
  const url =
    useSelector((state: RootState) => state.payments.paymentData?.url, shallowEqual) || "";
  const item =
    useSelector((state: RootState) => state.payments.paymentData?.item, shallowEqual) ||
    defaultItem;

  const { data } = useGetJobByIdQuery({ id: item.id }) as { data: JobsFindData | undefined };
  ...
  return (
    <>
      {data && item ? (
        <div className="mt-[100px] mb-[132px] px-[124px] gap-[200px] flex justify-between">
          ...
          <div className="border border-[#0612241a] rounded">
            <div className="bg-[#0612240e] text-[#4a4a4a] w-[400px] p-5">
              <div className="flex gap-3 w-[60px] items-center mb-[10px]">
                <Image
                  ...
                  width={100}
                  height={1}
                  src={data.publicationsFind[0].imgUrl}
                  alt=""
                />
                <h4 className="font-semibold text-left mb-[4px] text-base">
                  {data.publicationsFind[0].title}
                </h4>
              </div>
            ...
            <div className="bg-white text-[#4a4a4a] w-[400px] p-5">
              ...
              <button
                onClick={isMercadoPagoChecked ? () => (window.location.href = `${url}`) : () => {}}
                className={`w-full mt-5 border-none p-2.5 h-12 rounded text-white font-medium flex items-center justify-center ${
                  isMercadoPagoChecked ? "bg-sky-500" : "bg-[#3C65F5]"
                } transition-all cursor-pointer duration-300 ease-in-out opacity-100 hover:opacity-80 text-center`}>
        ...
        <div className="w-full flex flex-row gap-2 justify-center items-center mt-[100px]">
