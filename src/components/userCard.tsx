...
const UserCard = ({ id, name, city, description, skills }: IUser) => {
  ...
  return (
    <div className="flex items-center justify-between w-80 flex-col m-4 shadow-2xl">
      <div className="bg-indigo-600 rounded-t-md text-slate-100 pt-2 pb-2 relative w-full text-center h-96 flex flex-col justify-around">
          ...
          <span>
            {FavClicked ? (
              ...
              <button
                ...
                <GoHeartFill
                  size={30}
                  className="text-indigo-900 hover:text-indigo-300 transition-all duration-300"
                />
              </button>
            )}
          </span>
        </div>
        <h3 className="font-bold text-lg text-white my-1">{name}</h3>
        <h6 className="text-white font-bold">{city}</h6>
        <p className="text-sm	leading-5 text-white mt-1 mb-4">{description}</p>
        <div className="">
          <button className="bg-white border mr-2 w-28 text-indigo-600 border-slate-300 font-medium py-2 px-4 rounded hover:bg-indigo-300 transition-all duration-300">
            Message
          </button>
